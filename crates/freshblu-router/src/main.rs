...
async fn main() -> anyhow::Result<()> {
    ...
    let database_url = std::env::var("DATABASE_URL")
        .expect("DATABASE_URL must be set for router");
    ...
    let inner_store: freshblu_store::DynStore = Arc::new(
        freshblu_store::postgres::PostgresStore::new(&database_url).await?,
    );

    // Wrap with Redis cache
    let store: freshblu_store::DynStore = Arc::new(
        freshblu_store::cache::CachedStore::new(inner_store, &redis_url).await?,
    );
