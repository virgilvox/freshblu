...
import { FreshBluClient } from './client';
...
describe('FreshBluClient', () => {
  const client = new FreshBluClient(BASE);

  it('sets credentials and sends auth header', async () => {
  ...
  it('sends a message', async () => {
    await expect(client.sendMessage({ devices: ['target-1'] })).resolves.not.toThrow();
  });

  it('broadcasts', async () => {
    await expect(client.broadcast({ topic: 'test' })).resolves.not.toThrow();
  });

  it('creates a subscription', async () => {
    const sub = await client.createSubscription('test-uuid-1234', 'emitter-1', 'broadcast-sent');
    expect(sub.type).toBe('broadcast-sent');
  });

  it('lists subscriptions', async () => {
    const subs = await client.listSubscriptions('test-uuid-1234');
    expect(Array.isArray(subs)).toBe(true);
  });

  it('deletes a subscription', async () => {
    await expect(client.deleteSubscription('test-uuid-1234', 'emitter-1', 'broadcast-sent')).resolves.toBeUndefined();
  });
